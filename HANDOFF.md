...
| **虚拟模块数** | 8 个（已定型，不再新增） |
| **测试基线** | Playwright **55/55** 通过（[release-gate-task-005.mjs](release-gate-task-005.mjs)） |
| **构建基线** | 1664 模块，约 2.4s（gzip 主包 41.89 KB） |
...
**重要说明**：
- **RC4~RC7 候选页面顺序未锁定**。新 AI 在用户批准 RC4 后应主动询问用户：「RC4~RC7 的页面顺序与目标是什么？是否按 Skills → Resume → Interview → AiPractice 顺序推进，或需要调整？」
...
**RC3.3 IA Review 给 RC4+ 的 5 项 P2 建议（仅记录，不在 RC3.3 修改）：**

| # | 建议 | 建议时机 |
|---|---|---|
| 1 | 统一 About subtitle 与 Resume 开场白 framing | RC5（Resume 重构） |
| 2 | 评估 NavBar 顺序是否优先复试导师场景 | RC4+（用户决策） |
| 3 | 评估 4 个子页面（Skills/Interview/AiPractice/Resume）`page__hint` 是否应统一迁移至 SSOT 模式 | RC4-RC7（每页重构时） |
| 4 | 评估 3 个子页面 `.xxx__header` CSS 是否提取为 `.page__header` 工具类 | RC8（Final Review） |
| 5 | 监控 Skills 软件工程实践 vs Projects 技术亮点 vs Resume 工程能力 三处能力描述是否在 RC4+ 出现过度重复 | RC4-RC7（每页重构时） |
...
**RC4 开始前必须由用户决定**：
1. RC4 的具体页面与范围（候选之一是 Skills 页深化重构）
2. 是否先推送 RC3.1+RC3.2+RC3.3 共 3 个 commit 到 origin/master
3. 是否升级版本号到 v2.1.0（RC3 已完成 About 页面重构这一独立功能模块）
4. 是否参考 RC3.3 IA Review 的 5 项 P2 建议（详见 §6.2）

**预期工作模式**（参考 RC3 流程）：
1. 数据层重构（如需要扩展 frontmatter 字段）
2. 视觉与交互重构（如需要重构页面布局）
3. Final Review & Release（Code/Design/Performance/IA 局部审计 + 文档同步）
...
```
═══════════════════════════════════════════════════════════════
  Portfolio v2.0 长期 Roadmap（RC4 → RC8）
═══════════════════════════════════════════════════════════════
...
3. **每个子阶段完成后必须执行三项验证**（typecheck + build + Playwright）
4. **每个子阶段完成后必须输出 Report**，等待用户批准才能进入下一子阶段
5. **RC3.3 IA Review 已完成**，5 项 P2 建议供 RC4+ 决策参考（详见 §6.2）
